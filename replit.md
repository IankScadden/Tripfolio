...
- **AI Travel Assistant**: Context-aware chatbot powered by OpenAI that helps users estimate travel costs. Appears as a floating button throughout the app (except on shared/public trip pages). Requires sign-in to use - non-authenticated visitors see a sign-in prompt. Users can ask questions like "What does a meal cost in Madrid?" and receive specific price estimates. When the AI suggests a cost, users can click "Add to Budget" to instantly add that expense to their trip. Uses streaming responses for real-time interaction. AI usage is tracked per user with tier-based limits. The chatbot is intentionally hidden on shared trip links (/share/:id) and explore trip detail pages (/explore/:id) to keep the focus on the trip content when viewing public/shared trips.
- **Subscription Tiers**: Two-tier system for AI access - Free tier (3 AI uses per day, resets daily) and Premium tier ($2/month for unlimited AI and trips). Integrated with Stripe for payment processing, including checkout sessions and customer portal for subscription management. Promo codes can be redeemed for premium access. Users see remaining daily AI uses and can upgrade via modal with promo code support.
- **Tip Jar with Creator Payouts**: Visitors can tip trip creators via "Buy Me a Coffee" button on public trip pages (/explore/:id and /share/:shareId). Supports fixed amounts ($3, $5, $10) via Stripe Checkout. Creators can connect their Stripe account via Profile Settings to receive tips directly (85% to creator, 15% platform fee). Uses Stripe Connect Express accounts for easy onboarding. Success page at /tip-success shows thank you message. Tip dialog shows destination messaging (whether tips go to creator or platform).
