...
## Recent Changes
- 2024-10-19: **Fixed Expense Date Persistence Bug** - Expenses now store both `date` (calendar date) and `dayNumber` fields. When trip dates are edited, dayNumbers are automatically recalculated based on stored dates, so expenses stay on their original calendar dates instead of shifting with day numbers. Implemented `recalculateExpenseDayNumbers` storage method called automatically when trip startDate changes via PATCH /api/trips/:id. All expense creation/updates from day-by-day view now include date field.
- 2024-10-19: **Multiple Transportation Entries Per Day** - Users can now add multiple transportation items on the same day (e.g., flight + local transportation + intercity train). Transportation section displays as a list with "+ Add Transportation" button always visible. Each item shows icon, type label, name, cost, booking URL (for flights/intercity), and notes (for local). Items have three-dot menu with Edit/Delete options. Transportation state managed as array similar to activities pattern. Supports adding, editing, and deleting individual transportation entries. Backend creates individual expense records for each transportation item with proper dayNumber linking. Fixed critical persistence bug by snapshotting arrays before mutations to prevent query invalidation race condition.
- 2024-10-19: **Unified Transportation UI in Day-by-Day Layout** - Replaced separate "Local Transportation" and "Travel to Next City" sections with a single unified "Transportation" section featuring a dropdown selector. Users now choose from three transportation types: "Flight", "City to City Transportation", or "Local Transportation". The form adapts based on selection: flights and intercity show name/cost/URL fields, while local transportation shows name/cost/notes fields. Fixed critical bugs: (1) added `dayDetailData` to dependency array to ensure local transport notes load correctly, (2) added String() conversion for all cost fields to handle numeric-to-string type conversion from database. Load logic prioritizes flight > intercity > local when detecting which transportation type exists. Improves UX by simplifying the interface and reducing decision complexity.
- 2024-10-18: **Added Expense Edit Feature** - Replaced "Delete" button with three-dot menu (MoreVertical icon) offering "Edit" and "Delete" options for all expense entries. Users can now edit existing expenses by clicking Edit, which opens the expense dialog in edit mode with pre-populated data. Updated AddExpenseDialog to support both add and edit modes with initialData prop. All expense categories (flights, transportation, lodging, activities, other costs) now have full CRUD functionality.
- 2024-10-18: **Simplified Destination Input** - Removed LocationIQ autocomplete feature and replaced with simple text input for destination field (placeholder: "e.g., Barcelona, Spain"). This provides better international travel support without API limitations. Latitude/longitude fields retained in database (set to null) for future map visualization feature.
